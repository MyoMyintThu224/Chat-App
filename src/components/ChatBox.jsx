...
//   );
// }
// React နဲ့လိုအပ်တဲ့ hooks များ import
import React, { useEffect, useRef, useState } from "react";

// Firebase database (db) နဲ့ authentication (auth) import
import { db, auth } from "../lib/firebase";

// Firestore method များ import
import {
  collection, addDoc, query, orderBy, onSnapshot,
  serverTimestamp, updateDoc, doc, setDoc
} from "firebase/firestore";

// Firebase auth မှ current user လဲဖို့
import { onAuthStateChanged } from "firebase/auth";
...
// Emoji picker component ကို import
import EmojiPicker from "emoji-picker-react";

export default function ChatBox() {
  const [user, setUser] = useState(null); // ✅ User info state
  const [text, setText] = useState(""); // ✅ Message input text state
  const [messages, setMessages] = useState([]); // ✅ Message list state
  const [typingUsers, setTypingUsers] = useState({}); // ✅ Typing status state
  ...
  const scrollRef = useRef(); // ✅ Scroll to bottom ref

  // ✅ Firebase auth state change ရင် user သိရန်
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, setUser);
    return () => unsub();
  }, []);

  // ✅ Messages realtime update
  useEffect(() => {
    const q = query(collection(db, "messages"), orderBy("createdAt"));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      setMessages(snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })));
  ...
  // ✅ Typing status ကို realtime နဲ့ကြည့်
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "typingStatus"), (snapshot) => {
      const status = {};
      snapshot.forEach((doc) => {
  ...
  // ✅ Message အသစ်ရှိတိုင်း scroll လုပ်
  useEffect(() => {
    scrollRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [messages]);
  ...
  // ✅ Message ပေးပို့မယ်
  const handleSend = async () => {
    if (text.trim() === "" && !file) return; // စာမရှိ file မရှိရင် မပို့
  ...
  // ✅ Emoji ရွေးရင် စာထဲထည့်
  const handleEmojiClick = (emoji) => {
    setText((prev) => prev + emoji.emoji);
    setShowEmojiPicker(false);
  };

  // ✅ File ရွေးဖို့ handler
  const handleFileChange = (e) => {
    const selected = e.target.files[0];
    setFile(selected);
    ...
    <div className="flex flex-col h-[100dvh] w-full max-w-3xl mx-auto bg-gradient-to-br from-sky-100 via-blue-50 to-white dark:from-gray-800 dark:via-gray-900 dark:to-gray-950 text-gray-900 dark:text-white shadow-lg rounded-none sm:rounded-lg overflow-hidden">
      ...
      <div className="p-3 sm:p-4 bg-white dark:bg-gray-800 border-t border-blue-200 dark:border-gray-700 flex items-center gap-2 sm:gap-3">
        ...
        <input
          ...
          onChange={(e) => {
            setText(e.target.value);
            handleTyping(true); // ✅ Typing ဖြစ်နေပြသရန်
          }}
          onBlur={() => handleTyping(false)} // ✅ ထွက်ရင် typing မဖြစ်
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              handleSend(); // ✅ Enter နှိပ်ရင် ပေးပို့မယ်
              handleTyping(false);
            }
          }}
        />
